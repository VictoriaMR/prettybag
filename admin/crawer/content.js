...
var HELPER = {
	...
	getCategory: function() {
		var category = localStorage.getItem('crawer_category');
		if (category === null) {
			category = [];
		}
		return category;
	...
	getVersion: function() {
		var version = localStorage.getItem('crawer_version');
		if (version === null) {
...
var POP_PAGE = {
	init: function() {
		var _this = this;
		_this.version = HELPER.getVersion();
		var head = document.getElementsByTagName('head')[0];
		var script = document.createElement('script');
		script.src = common_url+'crawer/crawer.js?version='+_this.version;
		script.type = 'text/javascript';
		script.charset = 'utf-8';
		script.id = 'crawer_crawer_js';
		head.appendChild(script);
		script.onload=script.onreadystatechange=function() {
			var inpageCSS = common_url+'crawer/crawer.css?version='+_this.version;
			var head = document.getElementsByTagName('head')[0];
							...
							<div class="productAttLine">
								...
								<div class="fillin">
									<select name="bc_product_category" id="bc_product_category">
										...
										`+_this.getCategoryHtml(HELPER.getCategory(), 0)+`
									</select>
								</div>
								<div class="clear"></div>
							</div>
						...
						html += `<div class="clear"></div>
								<div class="productMainPic">
									...
									<div class="pdtPicHere" id="pdt_picture">
										<input type="hidden" name="bc_product_img" id="bc_product_img" value="`+data.pdt_picture.join(',')+`"/>`;
					...
					if (data.des_picture) {
						html += `<div class="clear"></div>
								<div class="productMainPic">
									...
									<div class="pdtPicHere" id="pdt_picture">
										<input type="hidden" name="bc_product_img" id="bc_product_des_picture" value="`+data.des_picture.join(',')+`"/>`;
	...
	getCategoryHtml: function(category, lev) {
		var html = '';
		for (var i=0;i<category.length;i++) {
			...
			if (category[i].son.length && category[i].son.length > 0) {
				html += '<option disabled="disabled" value="'+category[i].category_id+'">'+padding+category[i].name+'</option>';
				html += this.getCategoryHtml(category[i].son, lev + 1);
			} else {
				html += '<option value="'+category[i].category_id+'">'+padding+category[i].name+'</option>';
			}
		}
		return html;
	},
	...
	initPdtImgValue: function() {
		var pobj = document.getElementById('pdt_picture');
		var imgValueObj = document.getElementById('bc_product_img');
		if (imgValueObj === null) {
			pobj.innerHTML += '<input type="hidden" name="bc_product_img" value=""/>';
			imgValueObj = document.getElementById('bc_product_img');
		}
